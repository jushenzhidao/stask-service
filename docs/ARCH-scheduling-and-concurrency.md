...
> **注（2026-09-11）：本表中的 `st:tick` 与 `st:batch:meta:{key}` 两项未落地**，
> 实际使用的键与替换理由见 §3.3 末尾的「实现现状」。读本节时请连带读那段，
> 否则会照着不存在的键去查 Redis。
...
> **实现现状（2026-09-11）：ticker 与本节设计稿有四处差异，均为有意简化，逐条说明。**
>
> 1. **没有 `st:tick` 重入锁**——§3.2 键表里那一项未落地，全仓无 `K_TICK`。
>    多副本安全改由两件事共同保证：
>    ① **scheduler 单副本**（`docker-compose.yml` 把 scheduler 与 worker 放进同一个
>    容器，注释写明「多份会重复触发 cron」），这是真正防重复触发的那道约束；
>    ② **放行侧原子摘取**——`LUA_BATCH_CLAIM` 在一个 EVAL 内 `DEL` 成员键 +
>    `ZREM` 到期键，摘到才放行，后到者拿到空列表直接返回。也就是说在本设计里
>    「被重复触发」本身就是**幂等**的，抢锁并不能替代 ① 的作用。
> 2. **入口形态不同**：不是「一次投递的自驱动 15s 常驻循环」，而是
>    **taskiq cron `*/1 min` + 函数内自旋 4×15s**（`batching._TICK_ROUNDS` /
>    `_TICK_INTERVAL`）。精度等价（≈15s）、跑满一分钟即退出、不引入常驻线程，
>    但**锁与循环边界随之不同**：每轮 cron 都是新实例，不存在「持锁的常驻循环」。
> 3. **批次超时的检查方式换了**：设计稿第 4 步是「扫 `st:batch:meta:*` 的 deadline」，
>    实现改为**到期 ZSET `st:batch:due`**（member = 归组键，score = 放行时刻），
>    `tick_once` 用 `ZRANGEBYSCORE ... LIMIT` 取候选。理由是键空间扫描与批次数量
>    同阶，而 ZSET 取候选是 O(log N + M)，且与 `st:due` 的到期通道**共用一套索引语义**。
>    因此 §3.2 键表里的 `st:batch:meta:{key}` HASH 未落地，批次参数改由
>    `tasks.data`（`batch_size` / `batch_wait` / `batch_due_at` / `batch_key`）承载
>    ——每条成员行自带参数，重建索引时不必回查批次元数据。
> 4. **扫描范围比设计稿宽**：`tick_once` 除「批次 T 触发」与「到期放行」外，
>    还驱动**延迟任务到点**与**占槽失败重排**。这三件事共用一只 ticker 只是实现复用，
>    语义彼此无关，所以止血开关必须精确——`batch_enabled` 只跳批次放行那一段
>    （原因：延迟任务等待期被 sweeper 豁免，没有第二条恢复路径）。见 `docs/SPEC.md §10.1`。
>
> 放行阶段的有界并发由 dynconf 项 `batch_release_concurrency` 承担（默认仍为 8，
> 但**可热改**，而不是与 `_SWEEP_CONCURRENCY` 绑定的写死常量）。
> **本节其余部分（精度论证、为什么不精确到点执行）仍然成立。**
