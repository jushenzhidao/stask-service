...
def test_sql_time_predicates_are_bare_columns():
    ...
    扫的是**整个 ``taskstore`` 包**（2026-09-13 由单文件切成包），逐模块剥掉模块
    docstring——它按设计会引述被禁的写法，不剥就会误报。
    """
...
@pytest.mark.parametrize("app_env,strict", [
    ("dev", False), ("development", False), ("local", False),
    ("test", False), ("testing", False), ("ci", False), ("DEV", False),
    ("prod", True), ("PRODUCTION", True),
    ("prodd", True),      # 拼错的 prod —— fail-open 时代会静默关掉全部校验
    ("online", True),     # 不认识的部署环境
    ("", True),           # 显式置空
])
def test_strict_env_is_fail_closed(monkeypatch, app_env, strict):
    """严格环境判定必须 **fail-closed**：白名单之外一律按生产处理。

    反过来的写法（只认 prod/production）是 fail-open：`APP_ENV=prodd` 这种拼错会
    **静默跳过全部致命启动校验**，而那正是这些校验要防的场景（2026-09-13 复核发现）。
    宽松必须显式命中白名单，才是安全的默认方向。
    """
    monkeypatch.setattr(settings, "app_env", app_env)
    assert main._is_strict_env() is strict


@pytest.mark.parametrize("app_env,should_raise", [
    ("dev", False), ("test", False), ("prod", True), ("prodd", True),
])
def test_callback_secret_missing_is_fatal_in_strict_env(
    monkeypatch, app_env, should_raise
):
    """空 ``CALLBACK_SECRET`` ⇒ 回调不带签名，与 AC-29「必须签名」冲突，不能静默。

    严格环境阻断启动（与 ``CHANNEL_ID`` 同一口径）；宽松环境只告警，本地/单测不受影响。
    """
    monkeypatch.setattr(settings, "app_env", app_env)
    ...
    if should_raise:
        with pytest.raises(RuntimeError, match="CALLBACK_SECRET"):
            main._check_callback_secret()
    else:
        main._check_callback_secret()       # 宽松环境只告警


def test_callback_secret_present_is_silent(monkeypatch):
    """配了密钥就什么都不说——这条检查只针对「漏配」。"""
    monkeypatch.setattr(settings, "app_env", "prod")
    monkeypatch.setattr(settings, "callback_secret", "s3cret")
    main._check_callback_secret()
