"""用户令牌会话：**task_id → 用户 sk** 的唯一查询处。

为什么必须存：worker 执行时请求上下文早已结束，而调用 new-api relay
必须带**用户本人的 sk**（渠道选择、配额扣费、限流全靠它识别身份）。

安全口径（红线）：
- 只放 Redis（AOF everysec），**绝不**落 tasks 表、不进日志、不出任何
  HTTP 响应（ops 诊断端点只暴露存在性与剩余 TTL）；
- TTL ``ST_SK_SESSION_TTL_SECONDS``（默认 48h），终态立即清除；
- Redis 丢失的最坏后果是任务无法执行 → 超时 → 对账收敛为 FAILURE。
  本服务零资金动作，不会造成钱款损失。
"""
